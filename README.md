...
| | **第二部分　動手：怎麼做** | | |
| 5 | [類比、數位與 Buffer](chapters/05-digital-audio.md) | 取樣與量化、取樣率與混疊、位元深度、buffer 與延遲 | 中 |
| 6 | [Pure Data 入門](chapters/06-puredata-intro.md) | 歷史、視覺化程式語言、各版本比較與安裝 | 中 |
| 7 | [Pure Data 的物件](chapters/07-puredata-objects.md) | 訊號／音訊物件、hot & cold inlet、執行順序、UI 物件 | 中 |
| 8 | [Pure Data 聲音合成](chapters/08-puredata-synthesis.md) | 加法／減法／調變合成、包絡、取樣、延遲與殘響 | 長 |
| 9 | [Max/MSP 的使用](chapters/09-maxmsp.md) | 介面、數據處理、MSP 音訊、互動裝置、案例研究 | 長 |
| | **第三部分　收束** | | |
| 10 | [兩個工具的比較與選擇](chapters/10-comparison.md) | 逐項比較、五個真實情境、什麼時候該離開這兩個工具 | 中 |
| 11 | [結論與學習資源](chapters/11-conclusion.md) | 三條線的回顧、未來趨勢、書單與下一步 | 中 |
...
**想動手做，預算為零** → 第 1 章 → 第 6 章（裝 Pure Data）→ 第 7 章 → 第 8 章。
Pure Data 完全免費、跨平台，是學合成原理最直接的路。第 8 章做完，你會有一台自己的合成器。

**目標是展場裝置 / 劇場 / 接感測器** → 上面走完之後接第 9 章。
...
**不知道該用 Pd 還是 Max** → 直接看[第 10 章](chapters/10-comparison.md)。

**一學期的課怎麼排** → 見 [10.5 節](chapters/11-conclusion.md)。
...
│   ├── 04-game-audio.md
│   ├── 05-digital-audio.md
│   ├── 06-puredata-intro.md
│   ├── 07-puredata-objects.md
│   ├── 08-puredata-synthesis.md
│   ├── 09-maxmsp.md
│   ├── 10-comparison.md
│   └── 11-conclusion.md
