...
這兩招你現在就做得出來。垂直重混是幾個 `[tabplay~]` 同步播放，各自乘上一個 `[line~]` 控制的音量（7.6 節）；水平重排是用 `[select]` 決定下一段播哪個檔。遊戲業界花了十年才標準化的東西，其實是這門課第 6、7 章的組合。
...
**程序音訊（procedural audio）** 是指聲音不是預先錄好的檔案，而是在執行當下即時算出來的。這正是這門課從第 7 章開始一直在做的事。
