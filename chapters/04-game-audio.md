...
第 9 章 9.6 節分析互動作品時問了四個問題：輸入是什麼、映射怎麼設計、聲音怎麼產生、為什麼要即時。每一款遊戲都必須回答這四個問題，而且要在幾毫秒內回答完，一天回答幾百萬次。

除此之外，遊戲音樂還有兩個對這門課特別重要的理由：

1. 它是「限制下的創作」最好的教材。早期主機只有三個方波通道，作曲家必須在極端限制中做出音樂。而理解那些限制，就是在理解第 8 章的合成原理：晶片音樂等於一本硬體化的減法合成教科書。
2. 它是 Pure Data 最大的產業出口。2008 年的《Spore》把 Pure Data 整個嵌進遊戲裡跑程序音樂；這個做法後來被整理成 **libpd**，讓你在這門課學到的補丁，可以直接變成一款遊戲、一支 App 的聲音引擎（見 8.8）。
...
遊戲音樂的起點就在這裡。音樂從背景變成了遊戲狀態的聽覺化。用第 9 章的語言說，這是一條極簡但完整的映射：剩餘敵人數對應播放速度。
...
- **方波通道** = `[phasor~] → [>~ 0.5]`。方波只有**奇數倍泛音**（見 8.3 節），所以晶片音樂那種「中空、帶點簧片味」的音色是物理必然，不是風格選擇。
- **噪音通道** = `[noise~]`。負責所有的鼓、爆炸、腳步、風。
- **包絡產生器** = 8.6 節的 ADSR，只是被燒進硬體。

整台機器就是一台極簡的減法合成器。你在第 8 章學的每一個概念，在這裡都能一眼對上。
...
第 8 章 8.5 節講的 FM 合成，在 1980 年代中期進了遊戲機。
...
Mega Drive 那種銳利、帶金屬味、貝斯特別衝的聲音，就是 YM2612 的 FM 特性。回到 8.5 節：非整數比的調變比例產生非諧波泛音，聽起來就是金屬。

▶️ [Sega Genesis 與 FM 合成（Simon Hutchinson）](https://www.youtube.com/watch?v=Tkt3WyhwYWI)

Mega Drive 的音色可以在 Pd 裡用 8.5 節的補丁重建：調變比例設 1:1，再加一條快速衰減的包絡到調變深度上，那就是《音速小子》貝斯的骨架。
...
技術上這是第 8 章 8.7 + 8.8 節的硬體版：`[tabread4~]` 讀取樣、`[rev1~]` 加空間。真實樂器的聲音第一次進入家用主機。
...
這兩招你現在就做得出來。垂直重混是幾個 `[tabplay~]` 同步播放，各自乘上一個 `[line~]` 控制的音量（8.6 節）；水平重排是用 `[select]` 決定下一段播哪個檔。遊戲業界花了十年才標準化的東西，就是這門課第 6、7 章的組合。
...
**程序音訊（procedural audio）** 是指聲音不是預先錄好的檔案，而是在執行當下即時算出來的。這也是這門課從第 8 章開始一直在做的事。
...
這是本課程最直接的產業接點：你在第 6、7 章寫的補丁，透過 libpd 可以原封不動變成一款手機遊戲的聲音引擎。這也是 Pure Data 相對 Max/MSP 的優勢（見 9.7 節對照表），它可以被嵌入，Max 不行。
...
二、映射就是設計。
《Space Invaders》只有一條映射（敵人數 → 速度），但它是整個遊戲張力的來源。第 9 章 9.5 節說過「把數值變順往往比接得上更重要」，遊戲產業用四十年證明了這句話。
...
- 相關章節：合成原理見[第 8 章](08-puredata-synthesis.md)；互動與映射見[第 9 章](09-maxmsp.md)
