...
補丁一大就會亂，Max 提供了幾個整理工具，最好從第一天就養成習慣：
...
- **對齊**：選取多個物件後 `Arrange` 選單可以自動對齊分佈。線接得整齊不只是美觀問題，直接影響你日後除錯的速度。
...
外加一個特別的第六種：**signal（訊號）**，只在 MSP 物件之間流動，走的是粗線（見 9.4 節）。

bang 是 Max 最抽象也最重要的概念。它本身不帶任何資料，帶的是時機，所有「什麼時候發生」的邏輯都由它驅動。
...
> **職業習慣**：只要一個 outlet 要接到兩個以上的地方，就插一個 `[t]`。這個習慣看起來瑣碎，但能省下日後好幾個小時的除錯時間。
...
| `[buffer~]` | **音訊**取樣資料 | 載入音檔、錄音、波表（見 7.4） |
...
`[sel]` 與 `[route]` 的差別要記住：**`[sel]` 輸出 bang（只在意「是不是」），`[route]` 輸出剩下的資料（在意「內容是什麼」）**。
...
`[cycle~]` 與 `[phasor~]` 的分工是 MSP 的核心心法：`[cycle~]` 產生聽得到的東西，`[phasor~]` 產生掃過去的位置。後者接到 `[wave~]`、`[index~]`、`[groove~]` 上，就能讀波表、讀取樣、掃描任何東西。
...
直接用 `[tgl]` 開關訊號會產生「喀」聲，原因和 Pd 一章（8.6 節）說明的完全相同，振幅的瞬間跳變本身就是一次點擊噪音。
...
原理與參數意義（比例、調變指數）和 Pd 那章 8.5 節完全相同，只是 `[osc~]` 換成 `[cycle~]`。一旦理解了合成的原理，換工具只是換物件名稱。
...
`[tapin~]` / `[tapout~]` 這一對，等同於 Pd 的 `[delwrite~]` / `[delread~]`。延遲時間的長短決定它是哪一種效果（梳狀濾波 / 鑲邊 / 合唱 / 回聲），對照表見 Pd 那章的 8.8 節，兩邊完全通用。
...
Max 的強項不在合成聲音（Pd 也能做），而是**把各種東西接在一起**：感測器、影像、網路、燈光、投影、Ableton Live。這是它在裝置藝術與劇場領域幾乎沒有對手的原因。
...
長笛與即時電子音樂的作品，在 IRCAM（法國龐畢度中心的音樂研究機構）以 4X 音訊工作站完成，Max 就是在這個現場誕生的。Miller Puckette 當時在 IRCAM 開發的軟體（後來以 Max Mathews 命名為 Max）就是為了解決這類作品的需求。
...
攝影機拍攝觀眾，系統偵測影像的變化量，把「身體的動作」轉換成聲音。Rokeby 自己的說法點出了核心：這個系統的目的不在於讓人操作它，而在於讓人在與它相處的過程中重新意識到自己的身體。

- **輸入**：攝影機影像（用 Max 重建的話就是 `[jit.grab]`）
- **映射**：影格差異運算 → 動作量與位置 → 對應到音高、密度、音色
- **可以學的地方**：它刻意讓映射**不完全可預測**。如果動作和聲音是一對一的死板對應，人五分鐘就玩膩了；留一點模糊與延遲，人反而會待上半小時。**這是互動設計上的重要一課：可預測性與吸引力常常是反比。**
...
- **可以學的地方**：當你發現自己在 Max 裡做的事情，用一般的音樂軟體也能做，而且更快。那大概表示你還沒用到 Max 的核心價值。Max 的優勢在於**做出別的軟體裡不存在的東西**。

---

## 9.7 給 Pure Data 使用者的對照表
...
> 更完整的比較、五個真實情境該怎麼選、以及什麼時候該兩個都不用，見[第 10 章](10-comparison.md)。

多數人並不會只用其中一個。常見的狀態是用 Pd 學原理與做嵌入式的東西，用 Max 做展場裝置與需要影像的專案。兩邊的原理共通，`~` 是訊號、hot inlet 觸發、右到左執行、訊號自動相加這些規則完全一樣，學會一個之後，另一個只是查對照表的事。
