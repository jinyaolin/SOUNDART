...
第 6 章 6.6 節分析互動作品時問的四個問題——輸入是什麼、映射怎麼設計、聲音怎麼產生、為什麼要即時——**每一款遊戲都必須回答它們，而且要在幾毫秒內回答完，一天回答幾百萬次。**

除此之外，遊戲音樂還有兩個對這門課特別重要的理由：

1. **它是「限制下的創作」最好的教材。** 早期主機只有三個方波通道，作曲家必須在極端限制中做出音樂。而理解那些限制，就是在理解第 5 章的合成原理——**晶片音樂是硬體化的減法合成教科書**。
...
## 7.2 沒有音效晶片的年代（1972–1977）

### 《Pong》(1972)：聲音是電路的副產品

Atari 的《Pong》沒有任何音效晶片。工程師 **Al Alcorn** 被要求加入聲音時，他做的事情是——**在機器既有的同步產生器（sync generator）電路裡，找出本來就存在的幾個頻率，把它們接到喇叭上**。

那個著名的「嗶」聲，不是設計出來的，是**被發現的**。

這件事值得在藝術課裡講：它是 Russolo《噪音的藝術》主張的另一種版本——**聲音材料不必是「音樂的」，機器本身運作時發出的東西就可以是素材**。

### 《Space Invaders》(1978)：史上第一段會回應玩家的音樂

Taito、西角友宏設計。遊戲底下有一個**四個音的下行循環**，不斷重複。

關鍵在於：**外星人被消滅得越多，這段循環就跑得越快。**

原本這是硬體的副作用——畫面上剩下的外星人越少，機器要處理的東西越少，跑得就越快。但西角友宏決定**留下**這個現象，於是它變成了設計：

- 玩得越好 → 敵人越少 → 音樂越快 → **越緊張**
- 這是一個負回饋的心理設計：你的成功製造你的壓力

> **這是遊戲音樂的原點**：音樂不再是背景，而是**遊戲狀態的聽覺化**。用第 6 章的語言說，這是一條極簡但完美的映射——「剩餘敵人數 → 播放速度」。
...
## 7.3 PSG 晶片時代（1980 年代前期）

**PSG（Programmable Sound Generator，可程式化聲音產生器）**是第一代真正的遊戲音源晶片。最常見的兩顆：
...
- **方波通道** = `[phasor~] → [>~ 0.5]`。方波只有**奇數倍泛音**（見 5.3 節），所以晶片音樂那種「中空、帶點簧片味」的音色是物理必然，不是風格選擇。
- **噪音通道** = `[noise~]`。負責所有的鼓、爆炸、腳步、風。
- **包絡產生器** = 5.6 節的 ADSR，只是被燒進硬體。

**整台機器就是一台極簡的減法合成器。** 你在第 5 章學的每一個概念，在這裡都能一眼對上。

### 限制長出來的技巧

只有三個通道，卻要同時有旋律、低音、和聲、打擊——作曲家發展出一整套壓縮技法：
...
- **脈衝寬度（duty cycle）**：方波的高低比例可調（12.5% / 25% / 50%）。改變比例就改變泛音結構——**這是唯一能調的「音色旋鈕」**。

---

## 7.4 NES / 紅白機（1983）：五個通道的語法
...
**三角波不能調音量**這件事，直接決定了 NES 音樂的低音永遠是那種「厚而平」的質感。**限制不是缺點，是風格的來源。**
...
- 剩餘時間不足時，整首曲子**升速**——《Space Invaders》那條映射的直系後代
...
Famicom 的卡帶可以**自帶額外音源晶片**（Konami VRC6、Namco 163、Sunsoft 5B 等），等於買遊戲送硬體升級。這是遊戲產業特有的現象——**音源規格不是固定的，是可以隨作品擴充的**。

---

## 7.5 FM 時代（1985–1993）

第 5 章 5.5 節講的 FM 合成，在 1980 年代中期進了遊戲機。
...
Mega Drive 那種銳利、帶金屬味、貝斯特別衝的聲音，就是 YM2612 的 FM 特性——**回到 5.5 節：非整數比的調變比例產生非諧波泛音，聽起來就是「金屬」。**

▶️ [Sega Genesis 與 FM 合成（Simon Hutchinson）](https://www.youtube.com/watch?v=Tkt3WyhwYWI)

> **課堂連結**：Mega Drive 的音色，你可以在 Pd 裡用 5.5 節的補丁重建。調變比例設 1:1、加一條快速衰減的包絡到調變深度上——那就是《音速小子》貝斯的骨架。
...
技術上這是第 5 章 5.7 + 5.8 節的硬體版：`[tabread4~]` 讀取樣、`[rev1~]` 加空間。真正的樂器聲音第一次進入家用主機。

代價是**音訊記憶體只有 64KB**——所以那個年代的作曲家全部都是壓縮取樣的專家。

### CD 音軌：品質的高峰，互動的低谷

PC Engine CD-ROM²（1988）、Mega CD（1991）、PlayStation（1994）帶來 CD 音質的配樂。

但這裡發生了一件**在課程脈絡下很值得討論的事**：

> **CD 音軌是預先錄好的。它不能隨玩家的行為改變。**

技術進步的同時，遊戲音樂**倒退回線性播放**——變成「配上去的背景音樂」，而不是《Space Invaders》那種會回應你的系統。

這個張力貫穿整個 1990 年代，也直接催生了下一節的東西。

---

## 7.7 iMUSE（1991）：讓音樂重新變成互動的
...
結果是玩家在場景之間走動時，音樂像一個現場樂手一樣跟著你走——**沒有淡出淡入，沒有斷點**。
...
> **這兩招你現在就做得出來。** 垂直重混 = 幾個 `[tabplay~]` 同步播放，各自乘上一個 `[line~]` 控制的音量（5.6 節）。水平重排 = 用 `[select]` 決定下一段播哪個檔（第 4 章）。**遊戲業界花了十年才標準化的東西，是這門課第 4、5 章的組合。**

---

## 7.8 中介軟體與程序音訊（2000 年代至今）
...
這兩套做的事情，本質上就是**把 iMUSE 的概念做成給音樂人用的圖形介面**：狀態機、參數、層、隨機化、空間音訊。**它們是視覺化程式環境**——你在這門課學到的「用連線建構邏輯」，換一個介面就能上手。

### 程序音訊：Pure Data 的產業出口

**程序音訊（procedural audio）** 是指**聲音不是預先錄好的檔案，而是在執行當下即時算出來的**。這正是這門課從第 5 章開始一直在做的事。
...
- **Brian Eno** 參與了細胞階段的生成式音樂設計（他正是「generative music」這個詞的推廣者）
...
> **這是本課程最直接的產業接點**：
> 你在第 4、5 章寫的補丁，透過 libpd，可以原封不動變成一款手機遊戲的聲音引擎。
> 這也是 Pure Data 相對 Max/MSP 的關鍵優勢（見 6.7 節對照表）——**它可以被嵌入，Max 不行。**

### 為什麼程序音訊仍然是少數

誠實地說：即使到今天，絕大多數遊戲的音樂仍然是錄好的檔案 + 中介軟體切換。原因很實際——
...
## 7.9 動手：做一個會回應狀態的音樂系統

用第 4、5 章的東西，做一個最小的**垂直重混**系統：
...
## 7.10 小結：遊戲音樂教會聲音藝術的三件事

**一、限制會長出風格。**
三個方波通道逼出了琶音和弦；三角波不能調音量造就了 NES 的低音質感；64KB 音訊記憶體訓練出一整代的取樣壓縮高手。**當你覺得 Pd 做不出某個聲音時，先別急著找外掛——限制裡通常有答案。**

**二、映射就是設計。**
《Space Invaders》只有一條映射（敵人數 → 速度），但它是整個遊戲張力的來源。第 6 章 6.5 節說過「把數值變順往往比接得上更重要」——遊戲產業用四十年證明了這句話。

**三、互動的技術問題，最後都是音樂問題。**
iMUSE 真正困難的地方不是程式，是**「在哪裡切才不難聽」**。垂直重混與水平重排都是音樂結構的問題，不是工程問題。

> 這也是為什麼這門課先講第 1、2 章才講工具：
...
- 相關章節：合成原理見[第 5 章](05-puredata-synthesis.md)；互動與映射見[第 6 章](06-maxmsp.md)
