...
| 類別 | 物件 | 用途 |
|---|---|---|
| 數值與運算 | `[int]` `[f]` | 儲存並輸出整數／浮點數 |
| | `[+] [-] [*] [/]` | 基本算術 |
| | `[expr $f1 * sin($f2)]` | 一行寫完複雜算式 |
| 控制訊號 | `[bng]` | 觸發事件 |
| | `[metro 500]` | 節拍器，定時觸發 |
| | `[tgl]` | 開關，輸出 0 或 1 |
| 資料結構 | `[pack f f]` / `[unpack]` | 打包與解包 |
| | `[list]` | 列表操作 |
| 條件與邏輯 | `[select 1]` | 條件符合就輸出 bang |
| | `[route start stop]` | 依標籤分流 |
...
Note:
注意：網路上很多教材寫 [delay~] 和 [reverb~]，這兩個在 Pd vanilla 裡不存在。第 7 章有完整的正名對照表。
...
Pd 裡有兩種流動方式，它們的時間尺度完全不同：

**訊號流（Signal Flow）**
音訊訊號的連續流動，以每秒 44100 次（或更高）的速率運行，經過 `[osc~]`、`[dac~]` 這類音訊物件。

**消息流（Message Flow）**
控制訊號與資料的流動，是離散的事件：按下按鈕、改變數值。經過 `[+]`、`[metro]`、`[tgl]` 這類控制物件。

搞混這兩者是初學者最常見的錯誤來源，主控台會直接報 `signal outlet connect to nonsignal inlet`。

---

## 執行順序：為什麼重要

Pd 是**資料流**語言。順序錯了結果就錯，而且錯得很難查。
