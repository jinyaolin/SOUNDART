...
Note:
這一頁值得停久一點。Buchla 的立場在藝術系的課裡比在音樂系的課裡更好懂。
...
- 傳的不是聲音，是**演奏事件**：note on/off、力度、控制器、音色切換
- **沒有專利費、沒有授權金**
...
**但它極難編程。** 六個 operator 互相影響，改一個數字聲音就面目全非，**沒有一個旋鈕對應一個聽得懂的效果**。結果：多數人只用內建音色。
...
3. 加一個 LFO 到濾波器截止頻率（5.4 節），做出自動掃頻
...
當你在 Pd 裡設計一個補丁時，你也在設計一個介面——
**你決定了什麼是容易的、什麼是困難的、什麼是不可能的。**
