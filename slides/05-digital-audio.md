...
理想帶限條件下，取樣率必須高於訊號最高頻率的兩倍。這就是**取樣定理（sampling theorem）**，也常稱為 Nyquist–Shannon 取樣定理。

Note:
實務上 ADC 之前會加抗混疊濾波器，先把過高的頻率濾掉，而不是指望輸入乖乖不超頻。
「兩倍」是理想帶限訊號的條件，不是隨便什麼訊號都成立，講到這裡就好。
下一張(往下按)把 Nyquist 頻率講清楚。第 8 章會直接用到這個詞。

--

## Nyquist 頻率

<img class="diagram" src="../images/diagrams/da-07b-nyquist.svg" alt="Nyquist 頻率:取樣率的一半">

取樣率的一半，就是這個系統能正確表示的最高頻率。

48 kHz 的 Nyquist 頻率是 **24 kHz**；44.1 kHz 是 **22.05 kHz**。人耳上限約 20 kHz，所以兩者都夠用。

Note:
兩個很容易混用的詞，講的時候要分開：
・Nyquist 頻率 = fs / 2，是「取樣系統」的性質，給定取樣率就決定了。
・Nyquist rate = 訊號最高頻率的兩倍，是「對取樣率的要求」，給定訊號才決定。
第 8 章講 Pd 的取樣率時用的是前者（44100 Hz 下約 22050 Hz），所以這裡要先建立。
超過 Nyquist 頻率的成分不會消失，而是被「折回」低頻區，這就是上一張看到的混疊。
...
Note:
留意術語重疊：Max 的 `[buffer~]` 指的是「載入音檔的記憶體」，跟這裡講的 I/O 緩衝不是同一件事。第 9 章會遇到。
...
<img class="diagram" src="../images/diagrams/da-17-tradeoff.svg" alt="即時演奏與非即時混音對 buffer 的相反需求">

固定開銷容易攤薄，也比較能容忍短暫的運算波動。代價是排隊的資料變多，等待跟著增加。
